...
*				Accountable organisations, region and district information 
...
var activityDim, sectorDim, accountableOrgDim, fundingOrgDim, aOrgTypeDim, fOrgTypeDim, sectorFundingDim, regionDim, districtsDim, allGrp, activityDimGrp, sectorDimGrp, accountableOrgDimGrp, aOrgTypeDimGrp, fundingOrgDimGrp, fOrgTypeDimGrp, fundingByAccOrgGrp, sectorFundingDimGrp, uniqueActivityGrp;
...
var activity_chart, activity_counter, accountable_org_chart, funding_org_chart, aorgtype_chart, forgtype_chart, regtable_chart, funding_by_sector_chart;
...
function initialiseCharts(error, data, fxdata) {
    ...
    try {
        ...
        //Get accountable organisations by region for counting purposes
        accountableOrgDim = cf.dimension(function(d) {
            //	if (d.AccountableOrg.trim() !==""){
					return d.AccountableOrg;
            //}
        });
        ...
        //Accountable Organisation type dimension
        aOrgTypeDim = cf.dimension(function(d) {
            /*if (d.AccountableOrg =="")*/
            return d.OrgType;
        });
        ...
        accountable_org_chart = dc.rowChart("#acc_org");
		...
		d3.queue()
			...
			.defer(drawActivitiesByAccountableOrganisationChart)
...
function drawActivityBySectorChart(callback){
	 sectorDimGrp = sectorDim.group();
		...
		/*Activity Chart definition starts here */
        activity_chart.width(400).height(250)
            .dimension(sectorDim)
            .group(sectorDimGrp)
            //.renderLabel(true)
            .colors([
        ...
        activity_chart.xAxis().tickFormat(function(v) {
            return "";
        });
		callback(null, activity_chart);
}
...
/**
 * Function to create crossfilter group and draw activities by organisation (accountable) row chart 
 *
 * @param  
 * @return 
 */
function drawActivitiesByAccountableOrganisationChart(callback){
	//accountableOrgDimGrp = accountableOrgDim.group();
	/*  accountableOrgDimGrp = accountableOrgDim.group().reduce(
			function(p, d) {
		...
		console.log(accountableOrgDimGrp.top(2)); 
	*/
		
	accountableOrgDimGrp = accountableOrgDim.groupAll().reduce(
		function(p, d) {		
			if (d.AccountableOrg[0] === "") return p;
			$.each(d.AccountableOrg.split(', '), function(idx, val) {
		...
		function(p, d) {
			if (d.AccountableOrg[0] === "") return p;    // skip empty values
				$.each(d.AccountableOrg.split(', '), function(idx, val) {
	...
	accountableOrgDimGrp.all = function() {
		var newObject = [];
		for (var key in this) {
	...
	/* Accountable Organisation Chart definition starts here */
	accountable_org_chart.width(400).height(250)
		.dimension(accountableOrgDim)
		.group(accountableOrgDimGrp)
		.elasticX(true)
		.data(function(group) {
			return group.top(15);
		})
	...
	accountable_org_chart.xAxis().tickFormat(function(v) {
		return "";
	});
	
	//Custom filter handler to deal with multi-valued data
	accountable_org_chart.filterHandler (function (dimension, filters) {
	...
	callback(null, accountable_org_chart);
}
...
/**
 * Function to create crossfilter group and draw activities by organisation type (Accountable) row chart 
 *
 * @param  
 * @return 
 */
function drawActivitiesByAccountableOrganisationTypeChart(callback){
        //aOrgTypeDimGrp = aOrgTypeDim.group();
        aOrgTypeDimGrp = aOrgTypeDim.group().reduce(
            function(p, d) {
                if (d.ActivityCode in p.Activities) p.Activities[d.ActivityCode]++;
                else {
                    p.Activities[d.ActivityCode] = 1;
		 ...
		 /*Accountable Organisation type pie chart starts here*/
        aorgtype_chart.width(180).height(180)
            .dimension(aOrgTypeDim)
            .group(aOrgTypeDimGrp)
            .colors([
                '#b3e2cd',
                '#fdcdac',
                '#cbd5e8',
	...
	countryMapData = Highcharts.geojson(Highcharts.maps['countries/tz/tz-all']),
			...
			function(p, d) {
				...
				$.each(d.Regions.split(', '), function(idx, val) {
					if (typeof(p[val]) === "undefined") {
						...
						p[val].RegionalAccountableOrgs = [];
						p[val].RegionalAccountableOrgCount = 0;
					...
					if(d.AccountableOrg !==""){
						if (d.AccountableOrg in p[val].RegionalAccountableOrgs) { 
							//p.RegionalAccountableOrgs[d.AccountableOrg]++;
							//p[val].RegionalAccountableOrgs.push(d.AccountableOrg);
						}
						else {
							p[val].RegionalAccountableOrgs.push(d.AccountableOrg);
							p[val].RegionalAccountableOrgCount++;
						}
					}
				})
				return p;
						...
						if (typeof(p[val]) === "undefined") {
							...
							if(d.AccountableOrg !==""){
								if (d.AccountableOrg in p[val].RegionalAccountableOrgs) { 
									...
									delete p[val].RegionalAccountableOrgs[d.AccountableOrg];
									p[val].RegionalAccountableOrgCount--;
								}
							}
						}		
        ...
        $.each(countryMapData, function(i) {
			...
			if (typeof regionalActivityGrp.value()[curRegion] !== "undefined"){
				...
				this.value2 = regionalActivityGrp.value()[curRegion].RegionalAccountableOrgCount;
                            ...
                            $.getScript('data/' + mapKey + '.js', function(){
									...
									function(p, d) {
										...
										$.each(d.Districts.split(', '), function(idx, val) {
											if (typeof(p[val]) === "undefined") {
												...
												p[val].DistrictAccountableOrgs = [];
												p[val].DistrictAccountableOrgCount = 0;
											...
											if(d.AccountableOrg !==""){
												if (d.AccountableOrg in p[val].DistrictAccountableOrgs) { 
													
												}
												else {
													p[val].DistrictAccountableOrgs.push(d.AccountableOrg);
													p[val].DistrictAccountableOrgCount++;
												}
											}		
										})
										return p;
												...
												if (typeof(p[val]) === "undefined") {
													...
													if(d.AccountableOrg !==""){
														if (d.AccountableOrg in p[val].DistrictAccountableOrgs) { 
															...
															delete p[val].DistrictAccountableOrgs[d.AccountableOrg];
															p[val].DistrictAccountableOrgCount--;
														}
													}
												}		
                                ...
                                $.each(regionMapData, function(i) {
									...
									if (typeof districtActivityGrp.value()[curDistrict] !== "undefined"){
										...
										this.value2 = districtActivityGrp.value()[curDistrict].DistrictAccountableOrgCount;
							...
							$.each(countryMapData, function(i) {
								try{
									...
									if (typeof (regionalActivityGrp.value()[this.properties['name']]) !== "undefined"){
										...
										redrawValue2 = regionalActivityGrp.value()[this.properties['name']].RegionalAccountableOrgCount;
                ...
                formatter: function() {
                    if (this.point.drilldown !== undefined) {
                        return '<b>Region:</b> ' + this.point.name + '<br>' +
                            ...
                            '<b>Accountable Organisation Count:</b> ' + this.point.value2 + '<br>' +
			...
			$.each(countryMapData, function(i) {
				try{					
					...
					if (typeof (regionalActivityGrp.value()[this.properties['name']]) !== "undefined"){
						...
						filteredValue2 = regionalActivityGrp.value()[this.properties['name']].RegionalAccountableOrgCount;
			...
			$.each(regionMapData, function(i) {
				try {
						...
						if (typeof districtActivityGrp.value()[this.properties['name']] !== "undefined") {
							...
							distFilteredValue2 = districtActivityGrp.value()[this.properties['name']].DistrictAccountableOrgCount;
...
function getFiltersValues() {	
	var filters = [
		{ name: 'sector', value: activity_chart.filters()},
		{ name: 'accorg', value: accountable_org_chart.filters()},
